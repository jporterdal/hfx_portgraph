...
- **Depends on** `typed-retrieval-tools` being implemented (not just proposed) — the retriever node imports `retrieve_by_year`, `retrieve_by_report`, and `retrieve_with_section_filter` directly; this change should not begin implementation until those functions exist and pass their own validation (`evals/typed_retrieval_validation.md`).
