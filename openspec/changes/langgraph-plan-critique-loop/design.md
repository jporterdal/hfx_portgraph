...
- **[Dependency, added 2026-08-31, resolved same day]** This bare `retrieve_by_report()` call — no year filter, and potentially all 8 v1 reports when `report_ids` is unspecified — was exactly the call shape `typed-retrieval-tools/design.md`'s Decision 7 amendment addresses: `retrieve_by_report()` without `years` used to pool all requested reports through one query with no per-report coverage guarantee (empirically, 1/3 and 3/4 coverage on `gq-022`/`gq-007` when called this way). Decision 7 has since been **implemented** as a fast-follow patch to `typed-retrieval-tools` (its tasks.md 6.1-6.5, same day) — `retrieve_by_report()` now fans out per `report_id` by default, so this dispatch path can be built directly against it. The old pooled behavior (no guarantee) is still available as `retrieve_with_report_filter()`, for a future dispatch path that wants `report_ids` as a pure scoping filter rather than a coverage requirement — not needed for this entity-only goal shape, which wants the guarantee.
