...
Provide year-, report-, and section-type-filtered retrieval functions over the existing Chroma child-chunk index — using metadata already stored per chunk — so callers can target evidence precisely instead of relying on a single undifferentiated similarity search, and validate evidence coverage directly against the golden eval set's `expected_evidence` fields.
